...
pub async fn estimate(
    ...
    let row = sqlx::query("SELECT fund_code FROM fund WHERE fund_code = $1")
        .bind(&fund_code)
        .fetch_optional(pool)
        .await;
    ...
    let source_name = q.get("source").map(|s| s.as_str()).unwrap_or("eastmoney");
...
pub async fn batch_estimate(
    ...
    for code in &fund_codes {
        ...
        let cache_valid = match (&row.estimate_nav, row.estimate_time) {
    ...
    if !need_fetch.is_empty() {
        ...
        for (code, row) in need_fetch {
            ...
            set.spawn(async move {
                ...
                match eastmoney::fetch_estimate(&client, &code).await {
                    Ok(Some(data)) => {
                        let now = Utc::now();
                        let _ = sqlx::query(
                            r#"
...
pub async fn batch_update_nav(
    ...
    for code in existing_codes {
        ...
        set.spawn(async move {
            let _permit = sem.acquire_owned().await.expect("semaphore");
            match eastmoney::fetch_realtime_nav(&client, &code).await {
                Ok(Some(data)) => {
                    let _ = sqlx::query(
                        r#"
                        UPDATE fund
...
pub async fn query_nav(
    ...
    let _ = sync_nav_history_for_date(pool, fund_id, body.fund_code.trim(), query_date).await;
    let history_row = sqlx::query(
        r#"
        SELECT unit_nav::text as unit_nav, nav_date::text as nav_date
        FROM fund_nav_history
...
async fn sync_nav_history_for_date(
    ...
    let data = eastmoney::fetch_nav_history(&client, fund_code, Some(nav_date), Some(nav_date)).await?;
    if data.is_empty() {
        return Ok(0);
    }
    ...
    for item in data {
        let inserted: bool = sqlx::query(
            r#"
            INSERT INTO fund_nav_history (id, fund_id, nav_date, unit_nav, accumulated_nav, daily_growth, created_at, updated_at)
