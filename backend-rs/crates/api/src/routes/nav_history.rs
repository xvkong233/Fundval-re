...
pub async fn batch_query(
    ...
    let mut query = sqlx::query(&sql).bind(&fund_codes);
...
pub async fn sync(
    ...
    for code in fund_codes {
        match sync_one(pool, &client, &code, start_date, end_date).await {
            Ok(count) => {
...
async fn sync_one(
    ...
    if effective_start.is_none() {
        let last = sqlx::query(
            "SELECT nav_date FROM fund_nav_history WHERE fund_id = $1 ORDER BY nav_date DESC LIMIT 1",
        )
        .bind(fund_id)
        .fetch_optional(pool)
        .await
    ...
    let data = eastmoney::fetch_nav_history(client, fund_code, effective_start, Some(effective_end)).await?;
    if data.is_empty() {
        return Ok(0);
    }
