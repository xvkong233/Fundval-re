...
pub fn hash_password(raw_password: &str) -> String {
    ...
    let mut out = [0u8; 32];
    pbkdf2_hmac::<Sha256>(raw_password.as_bytes(), salt.as_bytes(), iterations, &mut out);
...
pub fn verify_password(raw_password: &str, encoded: &str) -> bool {
    ...
    let Some(algorithm) = parts.next() else { return false };
    let Some(iterations_str) = parts.next() else { return false };
    ...
    let Ok(iterations) = iterations_str.parse::<u32>() else { return false };

    let Ok(expected) = STANDARD.decode(hash_b64.as_bytes()) else { return false };
