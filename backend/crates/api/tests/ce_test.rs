...
fn ce_increases_when_returns_improve() {
    let rf_annual_percent = 2.0;
    let ce_bad = compute_ce_from_navs(&[1.0, 1.00, 0.99, 1.00, 0.98], rf_annual_percent, 3.0).unwrap();
    let ce_good = compute_ce_from_navs(&[1.0, 1.01, 1.02, 1.03, 1.04], rf_annual_percent, 3.0).unwrap();
