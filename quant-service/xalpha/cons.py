...
try:
    from pyecharts.options import (  # type: ignore
        AxisOpts,
        DataZoomOpts,
        LegendOpts,
        TooltipOpts,
    ...
    AxisOpts = DataZoomOpts = LegendOpts = TooltipOpts = VisualMapOpts = object  # type: ignore
    _HAS_PYECHARTS = False
...
if os.environ.get("FUNDVAL_XALPHA_CALENDAR_SELFCHECK", "").strip() == "1":  # pragma: no cover
    calendar_selfcheck()
...
connection_errors = (
    ...
    Exception if requests is None else requests.exceptions.RequestException,
    Exception if requests is None else requests.exceptions.ConnectionError,
    Exception if requests is None else requests.exceptions.SSLError,
